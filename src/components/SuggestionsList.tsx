...
import { isSuggestionsData } from '../common/typeGuards';
...
export default function SuggestionsList({
	...
	useEffect(() => {
		...
		if (
			...
			currentSuggestion < isSuggestionsData(data).length - 1
		) {
			setCurrentSuggestion((current) => current + 1);
		}
	...
	return (
		<>
			{!loading && isSuggestionsData(data).length > 0 && (
				<ul className={styles.suggestionsList} ref={ulRef}>
					{isSuggestionsData(data)?.map((suggestion: any, index: number) => (
						<li
							className={
								styles.suggestion +
